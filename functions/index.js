...
/**
 * Cloud Function: getSignalOrNoise
 * - HTTPS function that accepts { userId, currentActivity, importantTasks }
 * - Calls Vertex AI (Gemini) with a strict prompt to return only JSON: { "verdict": "Signal" } or { "verdict": "Noise" }
 */
const VerdictSchema = z.object({
  verdict: z.enum(['Signal', 'Noise']),
});
 
function buildFocusPrompt({ currentActivity, importantTasks }) {
  return [
    "You are a productivity analyst. Determine if a user's 'currentActivity' is related to their 'importantTasks' (Signal) or a distraction (Noise).",
    "Respond ONLY with a JSON object: { \"verdict\": \"Signal\" } or { \"verdict\": \"Noise\" }.",
...
exports.getSignalOrNoise = functions.region(REGION).https.onRequest(async (req, res) => {
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
  res.set('Access-Control-Allow-Headers', 'Content-Type, Authorization');
  if (req.method === 'OPTIONS') return res.status(204).send('');
  ...
  try {
    ...
    const { userId, currentActivity, importantTasks } = req.body || {};
    if (!userId || !currentActivity) {
      return res.status(400).json({ error: 'Missing userId or currentActivity' });
    }
 
    const model = getGenerativeModel();
    const prompt = buildFocusPrompt({ currentActivity, importantTasks });
